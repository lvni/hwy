	...
	easemobim.chat = function ( config ) {
		...
		return {
			...
			, handleReady: function ( info ) {
				...
				if ( utils.isTop ) {
					//get visitor
					var visInfo = easemobim.config.visitor || {};
                    ...
                    console.log(prefix);
                    console.log(ext);
                    console.log(easemobim.config.extMsg);
					ext = easemobim.config.extMsg;
...
;(function(window, undefined) {
	...
	function getConfig() {
		if (utils.isTop) {
			...
			try {
                ...
                //add by yuliang 配置读取 2017/02/16
                config.domain = easemobim.config.domain;
                config.path = easemobim.config.path;
                config.visitor = easemobim.config.visitor;
                config.extMsg = easemobim.config.extMsg;
                config.staticPath = easemobim.config.staticPath;
                config.hideKeyboard = utils.convertFalse(easemobim.config.hideKeyboard);
                //add by yuliang 配置读取 2017/02/16 
			} catch (e) {
	...
	function initUI(config, callback) {
		...
		iframe.src = config.domain + '/webim/transfer.html?v=43.12.018';
